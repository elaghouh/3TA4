...
#define TGFX_LITE_DEMO_REVISION              0xD1010001 /* 0xAAXXYYZZ - 0xAA : Demo ID | 0xXXYYZZ : MajorMinorPatch Version number */
#define DEMO_TGFX_LITE_ADDRESS               0x080C0000
#define DEMO_TGFX_LITE_RES_ADDRESS           0x9049A000
...
K_ModuleItem_Typedef  TGFX_board =
{  
  3,
  (uint8_t*)"",
  &bmtgfx,
...
static const GUI_WIDGET_CREATE_INFO _aNoFileDialogCreate[] = {
  ...
  { TEXT_CreateIndirect, "Please Add binary files", ID_TGFX_TEXT_FILENAME_0, 15, 75, 180, 20, 0, 0x0, 0 },
  { TEXT_CreateIndirect, "For TGFX Lite demo", ID_TGFX_TEXT_FILENAME_1, 8, 95, 180, 20, 0, 0x0, 0 },
...
static const GUI_WIDGET_CREATE_INFO _aInvalidFileDialogCreate[] = {
  ...
  { TEXT_CreateIndirect, "For TGFX Lite demo", ID_TGFX_TEXT_FILENAME_4, 8, 95, 180, 20, 0, 0x0, 0 },
...
{
  WM_HWIN hItem;
  Demo_Header_t * pQspiDemoHeader = (Demo_Header_t *)DEMO_TGFX_LITE_RES_ADDRESS;
  Demo_Header_t * pDemoDemoHeader = (Demo_Header_t *)(DEMO_TGFX_LITE_ADDRESS + DEMO_TGFX_HEADER_ADDRESS_OFFSET);
   ...
   */
  if (pDemoDemoHeader->demo_id == TGFX_LITE_DEMO_REVISION)
  {
     ...
     */
    if (pQspiDemoHeader->demo_id == pDemoDemoHeader->demo_id)
    {
      result = FR_OK;
    }
  }
  ...
  if((card_detected) && (result != FR_OK))
  {
     ...
     * notify the user that the expected duration is around 1mn
     */ 
    if (pQspiDemoHeader->demo_id != TGFX_LITE_DEMO_REVISION)
    {
      if(!_ShowMessageBox(hWin, "WARNING", " TouchGFX Lite demo resources\ndownload, expected duration:\n 1 mn.\n\nCarry on?"))
      {
        return;
      }
    }
    ...
    result = f_open(&TGFX_file, (const char*)"TGFX_Lite_flash.bin", FA_READ);
    if(result == FR_OK)      
    {
      ...
      if(result == FR_OK)
      {
         ...
         */
        else if (SdDemoHeader.demo_id == TGFX_LITE_DEMO_REVISION)
        {
          ...
          PROGBAR_SetMinMax(hItem, 0, TGFX_file.fsize);
          WM_MakeModal(hItem);
          
          k_FlashProgram(hItem, &TGFX_file, DEMO_TGFX_LITE_ADDRESS);
    ...
    result = f_open(&TGFX_file, (const char*)"TGFX_Lite_resources.bin", FA_READ);
    if(result == FR_OK)
    {
      result = f_read(&TGFX_file, &SdDemoHeader, sizeof(Demo_Header_t), (void *)&numOfReadBytes);
      if(result == FR_OK)
