...
static void Startup(WM_HWIN hWin, U16 xpos, U16 ypos)
{
  ...
  else if(card_detected)
  {
    ...
    if(EWIZ_flash_file.fsize > 0)
    {
      ...
      PROGBAR_SetMinMax(hItem, 0, EWIZ_flash_file.fsize);
      WM_MakeModal(hItem);
    ...
    if(EWIZ_resources_file.fsize > 0)
    {
      ...
      hItem = PROGBAR_CreateEx((40-WM_GetWindowOrgX(hWin)), 110, 150, 20, hWin, WM_CF_SHOW, 0, ID_EWIZ_PROGBAR_ID);
      PROGBAR_SetMinMax(hItem, 0, EWIZ_resources_file.fsize);
      WM_MakeModal(hItem);
